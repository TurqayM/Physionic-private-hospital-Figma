...
const navbarClickList = document.querySelector(".navbar-click-list");
const icnMenu = document.querySelector(".icnMenu");
const activee = document.querySelectorAll(".activee");

icnMenu.addEventListener("click", () => {
  navbarClickList.classList.toggle("show-click");
  inputMobile.classList.remove("input-show");
});

const inputMobile = document.querySelector(".inputMobile");
const icnSearch = document.querySelector(".icnSearch");

icnSearch.addEventListener("click", () => {
  inputMobile.classList.toggle("input-show");
  navbarClickList.classList.remove("show-click");
});

for (var i = 0; i < activee.length; i++) {
  activee[i].addEventListener("click", () => {
    navbarClickList.classList.remove("show-click");
  });
}
